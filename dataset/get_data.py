...
def save_reindex(df, save_name):
    ...
    for i, id_ in enumerate(item_list):
        item_dict[id_] = i
    print('saving...')
    user_times = {id_: set() for id_ in user_list}
    item_times = {id_: set() for id_ in item_list}
    ...
    with open('user_time.txt', 'w') as user_file:
        for original_id, mapped_id in user_dict.items():
            times = ','.join(sorted(user_times[original_id]))
            user_file.write(f'{mapped_id}\t{times}\n')

    with open('item_time.txt', 'w') as item_file:
        for original_id, mapped_id in item_dict.items():
            times = ','.join(sorted(item_times[original_id]))
            item_file.write(f'{mapped_id}\t{times}\n')
